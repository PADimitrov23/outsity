...
This document provides an overview of the Outsity band website for developers and AI agents working on this codebase.

## Project Overview

A dark, artistic band website for **Outsity**, built with TanStack Start (React 19) and deployed on Netlify. Three main sections: Merch, Drops, Members — all placeholder-driven for easy editing.

## Key Conventions

- **Styling**: Tailwind v4 + CSS custom properties in `styles.css`. Palette tokens: `--black`, `--acid` (#d4ff00), `--red`, `--cream`, `--cream-dim`.
- **Fonts**: Google Fonts via `@import` in `styles.css`. Archivo Black = headings, Inconsolata = body.
- **Placeholder content**: Each page has a top-level `const` array (e.g. `MERCH_ITEMS`, `DROPS`, `MEMBERS`). Edit those arrays to add real content.
- **Aesthetic**: Brutalist underground. Grain overlay is a fixed `body::before` — do not move to a scrolling container.
- **Animations**: CSS-only `@keyframes`. Glitch uses `clip-path` on a `.glitch-clone` sibling.
- **Divider trick**: `gap: '1px'` + `background: '#1a1a1a'` on grid containers creates hairline dividers without per-card borders.

## Routes

| File | Route | Description |
|------|-------|-------------|
| `src/routes/__root.tsx` | (shell) | Fixed nav + footer, Google Fonts, grain overlay |
| `src/routes/index.tsx` | `/` | Hero, marquee, section cards |
| `src/routes/merch.tsx` | `/merch` | Merchandise grid |
| `src/routes/drops.tsx` | `/drops` | Featured drop + release list |
| `src/routes/members.tsx` | `/members` | Band members alternating layout |

## Adding New Routes

Create `src/routes/<name>.tsx` with `createFileRoute('/<name>')`. Manually add the nav link in `src/routes/__root.tsx` inside the `Nav` component's link array.
